$ReleasesInOperation = @()
$hashtest = @{key1 = "dsgfre"; key2 = "asdfre"}
$ReleasesInOperation.Length += $hashtest
$filename = "100001_TestMultiple.idw"
$file = Get-VaultFile -Properties @{"Name" = $filename}
$assocs.Name = Get-VaultFileAssociations -File $file._FullPath
Update-VaultFile -File $file._FullPath -Status "Work in Progress"
foreach($assoc in $assocs){
    Write-Host $assoc.Name
	if($assoc.mASTERId -in $ReleasesInOperation){
		Update-VaultFile -File $assoc._FullPath -Status "Work in Progress"
        Write-Host "$($assoc.Name) is changed"
	}
}
...
$filename = '138764.iam'
$assocs1 = Get-VaultFileAssociations -File $vfile._Fullpath -Dependencies
$vfile = Get-VaultFile -Properties @{"Name" = $filename}
$file = $vault.DocumentService.GetFileById($vFile.id)
$fileMain = $vault.DocumentService.GetFileById(29893)
$assocsMain = $vault.DocumentService.GetFileAssociationsByIds($fileMain.Id, "All", $false, "All", $false, $false, $false)
foreach($Id in $files.Id){
    $vfile = Get-VaultFile -FileId $ID
    Write-hoST $Id, $vfile.Name
    $fname = $vfile._Name.TrimEnd($vfile._Extension).TrimEnd('.').ToLower()
    if(@("idw","dwg") -contains $vfile._Extension){
        if(@("$fname.idw", "$fname.dwg") -contains $vfile._Name.ToLower() -and $fname -match "^[0-9]{6}$"){
            Write-Host 'Hurra!!'
        }
    }
}

foreach($Id in $assocs){
    $Path = $Id.FileAssocs.expectedVaultPath 
    $ExpectedPath =Split-Path $Path -leaf
    Write-hoST $Id.FileAssocs.Id,$ExpectedPath 
}
...
$filenames = @('100001_TestMultiple.idw','100001.idw','100001.iam')
$files = @()
foreach($filename in $filenames){
...
function PostFileStateChange($files, $successful)
{
	...
	foreach($sFile in $sFiles){
		...
		if(@("Design Approval","Minor revision", "Work In Progress", "For Review") -contains $file._OldState  -and $file.State -eq "Released")
		{
			...
			foreach($assoc in $assocs){
				if ($assoc.Id -in $Global:rollBacked){
					$releaseFileAborted = $true
				}
			}
			if($releaseFileAborted) { continue }
			...
			if(@("ipt","iam") -contains $file._Extension){
				...
				$releasedInOperation += "Discarded assembly $($file._Name)"
				if($drawing -ne $null){
			...
			if(@("idw","dwg") -contains $file._Extension) {
				$releasedInOperation += $file.Id
				Add-VaultJob -Name "KOSAN" -Description "'$($file._Name)' - Create PDF and upload to IFS" -Priority $priority -Parameters @{"EntityClassId"="File";"EntityId"=$file.Id;"EntityMasterId"=$file.MasterId; "ReleasesInOperation" = $releasedInOperation }
				$priority++
				<#if(@("$fname.idw", "$fname.dwg") -contains $file._Name.ToLower() -and $fname -match "^[0-9]{6}$"){
					$releasedInOperation += $file.Id
					Add-VaultJob -Name "KOSAN" -Description "'$($file._Name)' - Create PDF and upload to IFS" -Priority $priority -Parameters @{"EntityClassId"="File";"EntityId"=$file.Id;"EntityMasterId"=$file.MasterId; "ReleasesInOperation" = $releasedInOperation }
			...
			if(@("ipt","iam") -contains $file._Extension){
				$releasedInOperation += $file.Id
				Add-VaultJob -Name "KOSAN" -Description "'$($file._Name)' - release model in IFS" -Priority $priority -Parameters @{"EntityClassId"="File";"EntityId"=$file.Id;"EntityMasterId"=$file.MasterId; "ReleasesInOperation" = $releasedInOperation }				
				$priority++
			}#>
		}
	}
