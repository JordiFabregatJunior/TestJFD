#function Invoke-SyncProperties($File) {}
$filename = "Main.iam"
...
Write-Host -Text "Starting SyncProperties for $($File._Name)"
$propertySync.SyncProperties($vault, $vaultFile, "PropertySync", $true, [ref]$writeResults, [ref]$cloakedEntityClasses, $true)
			...
			try // if anything goes wrong from here on out, undo the checkout
			{
				...
				vltobj.PropWriteReq[] writeProps = compProps.Select(
					p => new vltobj.PropWriteReq()
					{
						...
						Val = ConvertInvariantStringToObject(p.Val == null ? "" : p.Val.ToString(), m_propDefsByEntityClassAndId[p.EntClassId][p.PropDefId].Typ)
					}
					).ToArray();
...
'@
if (-not ([System.Management.Automation.PSTypeName]'Autodesk.Vault.SDKTools.PropertySync').Type) {
    Write-Host $vaultFile.Name
	## Debug
	Add-Type -Path "C:\Program Files\Autodesk\Vault Professional 2019\Explorer\Autodesk.Connectivity.WebServices.dll"
	Add-Type -TypeDefinition $cs -ReferencedAssemblies @("C:\Program Files\Autodesk\Vault Professional 2019\Explorer\Autodesk.Connectivity.WebServices.dll")

	# Production
	[System.Reflection.Assembly]::LoadWithPartialName("Autodesk.Connectivity.WebServices")
	Add-Type -TypeDefinition $cs -ReferencedAssemblies @("Autodesk.Connectivity.WebServices")
}